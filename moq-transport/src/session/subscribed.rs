...
impl Subscribed {
    ...
    async fn serve_inner(&mut self, track: serve::TrackReader) -> Result<(), SessionError> {
        ...
        // Build the SubscribeOk params. Draft-16 dropped the dedicated
        // expires/group_order/content_exists/largest_location fields; the largest
        // object location is now carried inside `params` via set_largest_object.
        // BLO-10339 additionally advertises the per-track subgroup history window
        // when the publisher set one, so a subscribing relay can bound its
        // mirror's retention to ours rather than retaining unbounded. Both live in
        // the same KeyValuePairs carrier (distinct keys: LARGEST_OBJECT vs
        // SUBGROUP_HISTORY_GROUPS_PARAM=0x40), so a single params must be built —
        // do NOT reintroduce a second `let params` or the window is shadowed away.
