...
mod tests {
    ...
    // Lock the fixture surface the Shaka/JS parity port consumes: if a vector is
    // renamed, dropped, or added, this fails loudly rather than silently drifting
    // the cross-impl contract.
    #[test]
    fn packet_vectors_cover_shaka_fixture_surface() {
        let doc = build_document();
        ...
        assert_eq!(doc["sequences"].as_array().unwrap().len(), 1);
    }
