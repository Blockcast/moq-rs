...
mod tests {
    ...
    // Lock the fixture surface (vector names) the JS reassembler parity port
    // consumes — a rename/drop/add fails loudly instead of drifting silently.
    #[test]
    fn reassembler_vectors_cover_parity_surface() {
        let doc = build_document();
        let names: Vec<&str> = doc["vectors"]
            .as_array()
            .unwrap()
            .iter()
    ...
    // Out-of-order fragments (last arrives before first) reassemble by ascending
    // fragment_counter, not arrival order. RAP rides the first (counter 0) frag.
    #[test]
    fn out_of_order_fragments_sort_by_counter() {
        let mfu = reassemble(
            "out_of_order_fragments",
            &[
                Frag {
        ...
        assert_eq!(to_hex(&mfu.data), to_hex(b"XXXZZZ"));
        assert_eq!(mfu.mpu_sequence_number, 3);
        assert_eq!(mfu.fragment_count, 2);
        assert!(mfu.rap_flag);
        // timestamp = mpu_seq * 1000 (first-fragment-wins; all frags share mpu_seq).
        assert_eq!(mfu.timestamp, 3000);
    }

    // Three in-order fragments concatenate A|B|C.
    #[test]
    fn three_fragments_concatenate_in_order() {
        let mfu = reassemble(
            "three_fragments",
            &[
                Frag {
        ...
        assert_eq!(to_hex(&mfu.data), to_hex(b"ABC"));
        assert_eq!(mfu.fragment_count, 3);
    }

    // A single FI=0 fragment is itself a complete MFU (zero-copy fast path).
    #[test]
    fn complete_mfu_single_fragment() {
        let mfu = reassemble(
            "complete_mfu_fi0",
            &[Frag {
                mpu_seq: 1,
        ...
        assert_eq!(to_hex(&mfu.data), to_hex(b"complete"));
